...
function Icon({ name, className }: { name: 'pipeline' | 'playbook' | 'team' | 'admin'; className?: string }) {
  const paths = {
    pipeline: <><path d="M4 6h16" /><path d="M4 12h11" /><path d="M4 18h6" /></>,
    team: <><circle cx="9" cy="8" r="3" /><path d="M3 20a6 6 0 0 1 12 0" /><path d="M16 5.5a3 3 0 0 1 0 5" /><path d="M18 14.5a6 6 0 0 1 3 5.5" /></>,
  ...
  const body = (
    <>
        ...
        <nav className="px-3 mt-5 space-y-0.5">
          {ITEMS.filter(item => !('directorOnly' in item) || role === 'director').map(item => {
